...
- 設定・すべての登録姿勢・保存中の全統計履歴を移行用ZIPへエクスポート
...
モニターのカメラ映像を1.2秒以内に3回クリックすると、骨をはじくゲームが始まります。
両腕をカメラに映し、飛んでくる骨を腕で反射して鼻を守ります。右上のライフは2つで、
鼻に骨が当たると1つ減ります。耐えた秒数がスコアになり、約30秒で骨の数と速度がかなり増えます。
終了後はスコアを5秒表示し、自動で元のモニターに戻ります。「モニターに戻る」でも閉じられます。
顔が映らない間は進行を止め、5秒で終了します。画面の切り替え・最小化・監視停止でも終了します。
ゲームのスコアや骨格座標は保存しません。
...
1. 移行元の「設定」で、変更中の設定があれば「保存」を押します。
2. 「別端末への移行」の「全データをエクスポート」を押し、ZIPの保存先を選びます。
3. 「保存しました」と表示されるまでアプリを終了せずに待ち、ZIPを移行先へコピーします。
4. 移行先へ同じバージョン以降のPoseCareをインストールし、タスクトレイから完全終了します。
5. 移行先の `%LOCALAPPDATA%\PoseCare` フォルダーがあれば、別名に変更して退避します。
6. 新しい `%LOCALAPPDATA%\PoseCare` フォルダーを作り、ZIP内の `settings.json` と
   `posture_history.sqlite3` を直下へコピーしてPoseCareを起動します。

登録姿勢（通知対象・通知しない姿勢の全特徴量を含む）、設定、保存されている全期間の
判定区間・姿勢名・通知履歴を移行できます。統計画面の表示期間に制限されません。
監視中でも最新の判定区間とSQLiteのWALを反映したバックアップを作成します。
ZIPには形式・作成日時の情報と「移行手順.txt」も含まれます。

移行は既存データとの統合ではなく置き換えです。SQLiteの古い `-wal` / `-shm` ファイルが
混ざらないよう、既存フォルダーを退避してから復元してください。
削除済みの履歴は復元できません（通常の保存期間は400日）。姿勢推定モデルと更新キャッシュは
含まず、モデルは必要時に再ダウンロードします。Windowsログイン時の自動起動は移行先で
設定し直し、カメラ番号も確認してください。アプリ内のインポート機能はありません。
